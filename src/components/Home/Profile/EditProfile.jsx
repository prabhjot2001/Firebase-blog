...
const EditProfile = ({ editModal, setEditModal, getUserData }) => {
  ...
  return (
    <>
      <Modal modal={editModal} setModal={setEditModal}></Modal>
      <div className="fixed center w-[95%] md:w-[45rem] bg-white rounded-2xl mx-auto shadows z-20 p-5">
          ...
          <div className="flex gap-10 ">
            ...
            <div className="w-10 flex flex-col gap-4 items-center">
              <img
                ...
                alt=""
                className="w-10 h-10 object-cover rounded-full border-2 border-gray-400"
              />
            ...
            <div className="flex flex-col gap-4">
              <div className="flex gap-4 ">
                <button
                  onClick={openFile}
                  className="px-3 py-1  text-blue-700 border-2 border-blue-700 rounded-md"
                >
                  Update
                </button>
                <button className="px-3 py-1  text-red-700 border-2 border-red-700 rounded-md">
                  Remove
                </button>
              </div>
              <p>You can upload png, jpg, jpeg files.</p>
            </div>
          </div>
        </section>
        {/* footer */}
        <section className="mt-5 flex flex-col gap-8">
          <div className="">
            ...
            <input
              className="outline-none bg-gray-100 w-full p-2 rounded-lg focus:border focus:border-blue-500"
              type="text"
        ...
        <div className="flex gap-5 flex-row-reverse mt-8">
          <button
            onClick={() => saveForm()}
            className="bg-green-800 text-white rounded-full py-2 px-5"
          >
            Save
          </button>
          <button
            onClick={() => setEditModal(false)}
            className="bg-gray-600 text-white rounded-full py-2 px-5"
          >
            Cancel
          </button>
        </div>
