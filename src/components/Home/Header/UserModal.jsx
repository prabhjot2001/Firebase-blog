  ...
  return (
    <section className="absolute w-[23rem] p-6 bg-white right-0 top-[100%] shadows rounded-xl z-30 text-gray-700">
      <RxCross1
        ...
        className="absolute rounded-sm p-[2px] text-xl right-3 top-3 cursor-pointer hover:bg-gray-200"
      />

      <h2 className="text-gray-600 font-bold text-2xl mb-4 text-center">
      ...
      <div className="flex flex-col gap-6 border-b border-gray-400 pb-5">
        {userModal.map((item, i) => (
          <Link
            onClick={() => setModal(false)}
            className="flex items-center gap-4 text-gray-500 hover:ml-3 hover:text-gray-700 hover:font-semibold  transition-all duration-200"
            to={item.path}
            key={i}
          >
      ...
      <span>
        ...
        <span className="text-blue-700">{hiddenEmail(currentUser?.email)}</span>
      </span>
    </section>
  );
};
